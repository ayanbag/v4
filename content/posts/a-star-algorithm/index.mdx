...
**A * Algorithm** is one of the most popular computer algorithm widely used in Path-finding and Graph Traversal. This algorithm efficiently search and plots a path between multiple nodes, or points, on the graph. It is used in various applications, such as *Maps*.
...
**A * ** achieves better performance by using heuristics to guide its search. For example, on a map with many obstacles, path-finding from points **A** to **B** can be difficult. A robot, for instance, without getting much other direction, will continue until it encounters an obstacle. With **A * **, a robot would instead find a walkable path.

### Why A* Algorithm ?

**A * ** is based on using heuristic method to achieve optimality and completeness, and based the Best-First Algorithm.
...
To calculate the exact value of **h** , firstly we have to pre-compute the distance between each pair of cells before running **A * ** algorithm.
